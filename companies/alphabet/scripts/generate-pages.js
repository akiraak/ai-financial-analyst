// HTMLページ生成スクリプト（Alphabet Inc.用）
// ワークフロー仕様（analyze-and-visualize.md）に基づき、3つのHTMLファイルを生成する
// - docs/alphabet/index.html（ランディングページ）
// - docs/alphabet/quarters/index.html（四半期選択ページ）
// - docs/alphabet/quarters/template.html（四半期詳細テンプレート）
...
const TICKER = 'GOOGL';
const SLUG = 'alphabet';
const DESC = '検索・広告・クラウド。Google Services・Google Cloud・Other Bets セグメント別業績を可視化。';
  ...
  <div class="container" id="mainContent">
    ...
    <div class="section">
      ...
      <div class="chart-description">
        <p><span class="label">Google Services:</span> Googleの中核事業。Google Search（検索広告）、YouTube Ads（動画広告）、Google Network（パートナーサイト広告）の3つの広告収入に加え、Subscriptions, Platforms & Devices（Google One, YouTube Premium/TV, Pixel, Fitbit等）を含む。</p>
        <p><span class="label">Google Cloud:</span> Google Cloud Platform (GCP) およびGoogle Workspace (旧G Suite) のクラウドサービス事業。近年急成長し、2023年以降黒字化を達成。</p>
        <p><span class="label">Other Bets:</span> Waymo（自動運転）、Verily（ヘルスケア）、Wing（ドローン配送）等の先端技術事業。大規模投資を伴い、営業赤字が続いている。</p>
        <p><span class="label">セグメント損益:</span> Google Services、Google Cloud、Other Betsの3セグメント別に売上高・営業利益が開示されている。Alphabet-level Activitiesは未配分のコーポレートコスト。</p>
      </div>
    </div>
