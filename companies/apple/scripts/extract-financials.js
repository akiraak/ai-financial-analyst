// Apple press-release.htm から損益計算書データを抽出するスクリプト
...
// 抽出対象の行ラベルとマッピング
const ROW_MAPPINGS = [
  { patterns: [/^Total net sales/i], key: 'revenue' },
  { patterns: [/^Total cost of sales$/i], key: 'costOfRevenue' },
  { patterns: [/^Gross margin$/i], key: 'grossProfit' },
  { patterns: [/^Research and development$/i], key: 'researchAndDevelopment' },
  { patterns: [/^Selling, general and administrative$/i], key: 'sellingGeneralAndAdministrative' },
  { patterns: [/^Total operating expenses$/i], key: 'totalOperatingExpenses' },
  { patterns: [/^Operating income$/i], key: 'operatingIncome' },
  { patterns: [/^Other income.*net$/i], key: 'otherIncomeNet' },
...
/**
 * press-release.htm から損益計算書テーブルを解析
 */
function extractFromFile(filePath, fy, q) {
  ...
  $('tr').each((i, row) => {
    ...
    // 発行済株式数セクション検出
    if (label.match(/Shares used in computing earnings per share/i)) {
