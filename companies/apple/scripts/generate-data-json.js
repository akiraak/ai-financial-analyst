// 全データソースを統合して docs/apple/data.json + 四半期別data.json を生成する
// Apple Inc.用 — 株式分割調整なし（調整済みEPS）、SGA = sellingGeneralAndAdministrative
// セグメント: 地域別（Americas, Europe, Greater China, Japan, Rest of Asia Pacific）
//            製品別（iPhone, Mac, iPad, Wearables, Services）
const fs = require('fs');
...
const OUTPUT = path.join(ROOT, 'docs/apple/data.json');
const QUARTERS_DIR = path.join(ROOT, 'docs/apple/quarters');
...
function getNonOperatingIncome(d) {
  // incomeBeforeTax と operatingIncome の差分で算出
  if (d.incomeBeforeTax != null && d.operatingIncome != null) {
    return d.incomeBeforeTax - d.operatingIncome;
  }
  // otherIncomeNet が直接ある場合
  if (d.otherIncomeNet != null) return d.otherIncomeNet;
  return null;
}

// SGA（販売費及び一般管理費）を取得
// Apple は sellingGeneralAndAdministrative で一括計上
function getSGA(d) {
  if (d.sellingGeneralAndAdministrative != null) return d.sellingGeneralAndAdministrative;
  return null;
}

// 営業費用合計を計算（R&D + SGA）
function getTotalOperatingExpenses(d) {
  // totalOperatingExpenses がある場合はそれを使う
  if (d.totalOperatingExpenses != null) return d.totalOperatingExpenses;
  return null;
}
...
for (const fyStr of fys) {
  ...
  for (const q of ['Q1', 'Q2', 'Q3', 'Q4']) {
    ...
    quarters.push({
      ...
      // セグメント別売上（Apple: 地域別5区分 + 製品別5区分）
      segments: seg ? {
        americas: seg.americas ?? null,
        europe: seg.europe ?? null,
        greaterChina: seg.greaterChina ?? null,
        japan: seg.japan ?? null,
        restOfAsiaPacific: seg.restOfAsiaPacific ?? null,
      } : null,
      // 製品別売上（iPhone, Mac, iPad, Wearables, Services）
      productSegments: seg ? {
        iPhone: seg.iPhone ?? null,
        mac: seg.mac ?? null,
        iPad: seg.iPad ?? null,
        wearables: seg.wearables ?? null,
        services: seg.services ?? null,
      } : null,
      // B/S
      balanceSheet: bs ? {
        cashAndEquivalents: bs.cashAndEquivalents ?? null,
        marketableSecurities: bs.marketableSecurities ?? null,
        totalAssets: bs.totalAssets ?? null,
        totalLiabilities: bs.totalLiabilities ?? null,
        totalEquity: bs.stockholdersEquity ?? null,
        currentTermDebt: bs.currentTermDebt ?? null,
        commercialPaper: bs.commercialPaper ?? null,
      ...
      // セグメント営業利益（Apple: 地域別5区分）
      segmentProfit: segProfit ? {
        americas: {
          revenue: segProfit.americas ?? null,
          operatingIncome: null,
        },
        europe: {
          revenue: segProfit.europe ?? null,
          operatingIncome: null,
        },
        greaterChina: {
          revenue: segProfit.greaterChina ?? null,
          operatingIncome: null,
        },
        japan: {
          revenue: segProfit.japan ?? null,
          operatingIncome: null,
        },
        restOfAsiaPacific: {
          revenue: segProfit.restOfAsiaPacific ?? null,
          operatingIncome: null,
        },
      } : null,
      // 投資ポートフォリオ（Appleの場合は現在空データだが将来対応のため保持）
      investments: inv ? {
