...
// Apple Inc.用 — 株式分割なし（調整済みEPS）、SGA = sellingGeneralAndAdministrative
...
async function main() {
  ...
  quarters.forEach(({ fyStr, q, col }, i) => {
    ...
    if (d) {
      ws.getCell(3, col).value = d.revenue;          // Total net sales
      ws.getCell(6, col).value = d.grossProfit;       // Gross margin
      ws.getCell(9, col).value = d.researchAndDevelopment;
      ws.getCell(11, col).value = getSGA(d);          // Selling, general and administrative
      // 販管費合計 = R&D + SGA
      const sga = getSGA(d);
