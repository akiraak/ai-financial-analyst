// 生成されたxlsxの数値検証スクリプト（Apple Inc.用）
...
async function main() {
  ...
  const ws = wb.getWorksheet('AAPL業績');
  ...
  for (const { fyStr, q, col } of quarters) {
    ...
    const checks = [
      ...
      { row: 23, name: 'EPS', expected: d.epsDiluted, tolerance: 0.01 },
  ...
  for (let i = 0; i < quarters.length; i++) {
    ...
    const sga = getSGA(d);
    const yoySGA = yoyD ? getSGA(yoyD) : null;
    const opex = (d.researchAndDevelopment || 0) + (sga || 0);
    const yoyOpex = yoyD ? (yoyD.researchAndDevelopment || 0) + (yoySGA || 0) : null;
    ...
    if (sga && d.revenue) {
      formulaChecks.push({ row: 12, name: 'SGA売上比', expected: sga / d.revenue });
    }
    if (opex && d.revenue) {
