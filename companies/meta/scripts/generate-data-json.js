// 全データソースを統合して docs/meta/data.json + 四半期別data.json を生成する
// Meta Platforms用 — 株式分割調整なし、SGA = marketingAndSales + generalAndAdministrative
const fs = require('fs');
...
const OUTPUT = path.join(ROOT, 'docs/meta/data.json');
const QUARTERS_DIR = path.join(ROOT, 'docs/meta/quarters');
...
// SGA（販管費）を計算: marketingAndSales + generalAndAdministrative
function getSGA(d) {
  const ms = d.marketingAndSales ?? 0;
  const ga = d.generalAndAdministrative ?? 0;
  // 両方nullの場合はnullを返す
  if (d.marketingAndSales == null && d.generalAndAdministrative == null) return null;
  return ms + ga;
}

// 営業費用合計を計算（costOfRevenue以外の営業費用）
function getTotalOperatingExpenses(d) {
  // totalCostsAndExpenses がある場合はそれを使う
  if (d.totalCostsAndExpenses != null) return d.totalCostsAndExpenses;
  return null;
}
...
for (const fyStr of fys) {
  ...
  for (const q of ['Q1', 'Q2', 'Q3', 'Q4']) {
    ...
    quarters.push({
      ...
      // セグメント別売上（Meta: Family of Apps / Reality Labs）
      segments: seg ? {
        familyOfApps: seg.familyOfAppsRevenue ?? null,
        realityLabs: seg.realityLabsRevenue ?? null,
      } : null,
      // B/S
      balanceSheet: bs ? {
      ...
      // セグメント営業利益（Family of Apps / Reality Labs）
      segmentProfit: segProfit ? {
        familyOfApps: {
          revenue: segProfit.familyOfApps?.revenue ?? null,
          operatingIncome: segProfit.familyOfApps?.operatingIncome ?? null,
        },
        realityLabs: {
          revenue: segProfit.realityLabs?.revenue ?? null,
          operatingIncome: segProfit.realityLabs?.operatingIncome ?? null,
        },
      } : null,
      // 投資ポートフォリオ（Meta独自構造: nonMarketableBalance, marketableEquityBalance）
      investments: inv ? {
        nonMarketableBalance: inv.nonMarketableBalance ?? null,
        publiclyHeldBalance: inv.marketableEquityBalance ?? null,
      } : null,
    });
  }
}
