// HTMLページ生成スクリプト（Meta Platforms用）
// ワークフロー仕様（analyze-and-visualize.md）に基づき、3つのHTMLファイルを生成する
// - docs/meta/index.html（ランディングページ）
// - docs/meta/quarters/index.html（四半期選択ページ）
// - docs/meta/quarters/template.html（四半期詳細テンプレート）
...
const TICKER = 'META';
const SLUG = 'meta';
const DESC = 'Facebook・Instagram・WhatsAppを運営する世界最大級のSNS企業。広告収入が柱、Reality Labsでメタバースに投資。';
  ...
  <div class="container" id="mainContent">
    ...
    <div class="section">
      ...
      <div class="chart-description">
        <p><span class="label">報告セグメント（2区分）</span>: Meta Platformsは Family of Apps（FoA）と Reality Labs（RL）の2セグメントで報告している。</p>
        <p><span class="label">Family of Apps:</span> Facebook、Instagram、Messenger、WhatsApp、その他サービスで構成。主な収益源は広告収入。デジタル広告プラットフォームとして世界最大級のユーザーベースを持つ。</p>
        <p><span class="label">Reality Labs:</span> AR/VR向けのコンシューマーハードウェア（Meta Quest等）、ソフトウェア、コンテンツを提供。メタバース構想の中核セグメントだが、現在は大幅な営業赤字が続いている。</p>
      </div>
    </div>
