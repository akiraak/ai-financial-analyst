...
//
// Microsoftの会計年度: 7月〜6月
//   Q1: 7月-9月（四半期末 9/30）
//   Q2: 10月-12月（四半期末 12/31）
//   Q3: 1月-3月（四半期末 3/31）
//   Q4: 4月-6月（四半期末 6/30）
...
function getQuarterEndDate(fy, q) {
  ...
  switch (q) {
    case 'Q1': return new Date(fyNum - 1, 8, 30);  // 9月30日
    case 'Q2': return new Date(fyNum - 1, 11, 31);  // 12月31日
    case 'Q3': return new Date(fyNum, 2, 31);        // 3月31日
    case 'Q4': return new Date(fyNum, 5, 30);        // 6月30日
    default: return null;
  }
}
...
async function getStockPrice(targetDate) {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/MSFT?period1=${period1}&period2=${period2}&interval=1d`;
