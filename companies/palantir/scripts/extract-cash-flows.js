// Palantir press-release.html からキャッシュフローデータを抽出するスクリプト
// "Condensed Consolidated Statements of Cash Flows" テーブルを解析
// Palantirの決算プレスリリースはCF計算書を年次のみ掲載するため、
// Q4のみ年間累計が取得可能。Q1-Q3も掲載がある場合は取得する。
// 出力: cash-flows.json
...
const CF_ROW_MAPPINGS = [
  { patterns: [/^Net cash provided by operating activities$/i, /^Net cash used in operating activities$/i, /^Net cash provided by \(used in\) operating activities$/i], key: 'operatingCashFlow' },
  { patterns: [/^Net cash used in investing activities$/i, /^Net cash provided by \(used in\) investing activities$/i, /^Net cash provided by investing activities$/i], key: 'investingCashFlow' },
  { patterns: [/^Net cash used in financing activities$/i, /^Net cash provided by \(used in\) financing activities$/i, /^Net cash provided by financing activities$/i], key: 'financingCashFlow' },
  ...
  { patterns: [/^Stock.based compensation$/i], key: 'stockBasedCompensation' },
];
  ...
  for (const fy of fyDirs) {
    ...
    for (const q of qDirs) {
      ...
      if (data) {
        ...
        console.log(`  → CFデータなし（四半期のみの場合は正常）`);
      }
    }
  }
