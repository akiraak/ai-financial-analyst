// Palantir press-release.html から損益計算書データを抽出するスクリプト
...
function extractFromFile(filePath, fy, q) {
  ...
  $('tr').each((i, row) => {
    ...
    // 発行済株式数行の検出（ラベルに"Weighted-average"と"basic"/"diluted"を含む）
    if (label.match(/Weighted.average.*basic$/i)) {
      if (!('sharesBasic' in result)) {
        result.sharesBasic = firstValue;
        return;
      }
    }
    if (label.match(/Weighted.average.*diluted$/i)) {
      if (!('sharesDiluted' in result)) {
  ...
  for (const fy of fyDirs) {
    ...
    for (const q of qDirs) {
      ...
      if (data) {
        ...
        if (!data.netIncome && !data.netIncomeToCommon) console.warn(`  ⚠ Net income が見つかりません`);
      }
    }
  }
