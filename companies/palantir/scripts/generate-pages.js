// HTMLページ生成スクリプト（Palantir Technologies用）
// ワークフロー仕様（analyze-and-visualize.md）に基づき、3つのHTMLファイルを生成する
// - docs/palantir/index.html（ランディングページ）
// - docs/palantir/quarters/index.html（四半期選択ページ）
// - docs/palantir/quarters/template.html（四半期詳細テンプレート）
...
const TICKER = 'PLTR';
const SLUG = 'palantir';
const DESC = 'データ分析・AIプラットフォーム。Government・Commercial セグメント別業績を可視化。';
          ...
          for (const q of sorted) {
            ...
            if (q.isOutlook) {
              ...
              html += '<a href="' + dirName + '/" class="quarter-card">' +
                '<div class="quarter-card-label">Q' + q.q + '</div>' +
                '<div class="quarter-card-revenue">$' + (q.revenue / 1000000).toFixed(0) + 'M</div>' +
  ...
  <div class="container" id="mainContent">
    ...
    <div class="section">
      ...
      <div class="chart-description">
        <p><span class="label">報告セグメント（2区分）</span>: Palantir Technologiesは Government と Commercial の2セグメントで報告している。</p>
        <p><span class="label">Government:</span> 米国政府機関および同盟国の政府機関向けのデータ分析プラットフォーム。国防・情報機関を中心に、Gothamプラットフォームを提供。安全保障・防衛分野での強固な顧客基盤を持つ。</p>
        <p><span class="label">Commercial:</span> 民間企業向けのデータ分析プラットフォーム。Foundryプラットフォームを中心に、AIP（Artificial Intelligence Platform）を通じてAI/ML機能を提供。近年はAIPの急成長により売上が加速している。</p>
      </div>
    </div>
