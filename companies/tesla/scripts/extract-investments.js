// Tesla press-release.html / 10-Q / 10-K から現金・投資・負債データを抽出するスクリプト
// 出力: data/investments.json
//
// 抽出項目:
// - cashAndInvestments: 現金、現金同等物および投資（Cash, cash equivalents and investments）
// - shortTermDebt: 短期借入金（Current portion of debt and finance leases）
// - longTermDebt: 長期借入金（Debt and finance leases, net of current portion）
// - totalDebt: shortTermDebt + longTermDebt
// - netCash: cashAndInvestments - totalDebt
//
// データソース:
...
function extractFromPressReleaseText(text, headers) {
  ...
  // 現金・投資: "Cash, cash equivalents and investments" (新形式) or "Cash and cash equivalents" (旧形式)
  const cashAndInvestments = extractRow(/Cash(?:[,\s]+| and )cash equivalents(?:\s+and\s+investments)?/i, assetsSectionStart);

  // === LIABILITIES セクション ===
  const liabSectionStart = liabStart > 0 ? liabStart : (assetsSectionStart + 500);

  // 短期借入金: "Current portion of debt and finance leases"
  const shortTermDebt = extractRow(/Current portion of debt(?:\s+and finance leases)?/i, liabSectionStart);

  // 長期借入金: "Debt and finance leases, net of current portion"
  const longTermDebt = extractRow(/Debt and finance leases,?\s*net of current(?:\s+portion)?/i, liabSectionStart);
  ...
  for (let i = 0; i < numCols; i++) {
    ...
    if (cashAndInvestments && cashAndInvestments[i] != null) data.cashAndInvestments = cashAndInvestments[i];
    if (shortTermDebt && shortTermDebt[i] != null) data.shortTermDebt = shortTermDebt[i];
    if (longTermDebt && longTermDebt[i] != null) data.longTermDebt = longTermDebt[i];

    // totalDebt と netCash を計算
    if (data.shortTermDebt != null && data.longTermDebt != null) {
      data.totalDebt = data.shortTermDebt + data.longTermDebt;
    }
    if (data.cashAndInvestments != null && data.totalDebt != null) {
      data.netCash = data.cashAndInvestments - data.totalDebt;
    }
 ...
 * @returns {Object|null} { cashAndInvestments: xxx, shortTermDebt: xxx, ... }
 */
function extractFromSecFiling(filePath) {
  ...
  // 行マッピング定義（ラベル → 出力キー）
  const ROW_MAPPINGS = [
    { patterns: [/^Cash[,\s]+cash equivalents/i, /^Cash and cash equivalents$/i], key: 'cashAndInvestments' },
    { patterns: [/^Current portion of debt(?:\s+and finance leases)?$/i], key: 'shortTermDebt' },
  ...
  // totalDebt と netCash を計算
  if (result.shortTermDebt != null && result.longTermDebt != null) {
    result.totalDebt = result.shortTermDebt + result.longTermDebt;
  }
  if (result.cashAndInvestments != null && result.totalDebt != null) {
    result.netCash = result.cashAndInvestments - result.totalDebt;
  }
...
function main() {
  ...
  console.log('=== Step 1: プレスリリースから投資・負債データ抽出 ===');
  ...
  for (const fy of fyDirs) {
    ...
    for (const q of qDirs) {
      ...
      if (allData[fy] && allData[fy][q] && allData[fy][q].cashAndInvestments) continue;
      ...
      if (data && data.cashAndInvestments) {
        if (!allData[fy]) allData[fy] = {};
        allData[fy][q] = { ...(allData[fy][q] || {}), ...data };

        console.log(`    → Cash: ${data.cashAndInvestments}, Short-term Debt: ${data.shortTermDebt || 'N/A'}, Long-term Debt: ${data.longTermDebt || 'N/A'}`);
      } else {
        console.log(`    → 抽出失敗`);
      }
    }
  ...
  for (const fy of sortedFYs) {
    ...
    for (const q of sortedQs) {
      ...
      if (!d.cashAndInvestments) {
        missingCash++;
        console.warn(`  ⚠ ${fy}/${q}: Cash and Investments なし`);
      }
      if (d.shortTermDebt == null) {
        console.warn(`  ⚠ ${fy}/${q}: Short-term Debt なし`);
      }
      if (d.longTermDebt == null) {
        console.warn(`  ⚠ ${fy}/${q}: Long-term Debt なし`);
      }

      console.log(`  ${fy}/${q}: Cash=${d.cashAndInvestments || 'N/A'}, STD=${d.shortTermDebt || 'N/A'}, LTD=${d.longTermDebt || 'N/A'}, Total Debt=${d.totalDebt || 'N/A'}, Net Cash=${d.netCash || 'N/A'}`);
    }
  }

  // Step 5: FY/Qキーをソートして出力
  const sortedOutput = {};
  for (const fy of sortedFYs) {
    sortedOutput[fy] = {};
    const sortedQs = Object.keys(allData[fy]).sort();
  ...
  console.log(`\n出力: ${OUTPUT_PATH}`);
  console.log(`合計: ${totalQuarters} 四半期 (Cash欠落: ${missingCash})`);
}
