// 全データソースを統合して docs/tesla/data.json + 四半期別data.json を生成する
// Tesla, Inc.用 — 分析期間内の株式分割なし、epsDiluted をそのまま使用
// 営業外収支 = interestIncome + interestExpense + otherIncomeNet
// セグメント: Automotive, Energy Generation and Storage, Services and Other（固定3セグメント）
const fs = require('fs');
...
const OUTPUT = path.join(ROOT, 'docs/tesla/data.json');
const QUARTERS_DIR = path.join(ROOT, 'docs/tesla/quarters');
...
// segment-profit.json はオプション（存在しない場合はスキップ）
let segProfitData = {};
...
// investments.json はオプション（存在しない場合はスキップ）
let investmentsData = {};
...
// 営業外収支を計算（Tesla: interestIncome + interestExpense + otherIncomeNet）
function getNonOperatingIncome(d) {
  const ii = d.interestIncome ?? 0;
  const ie = d.interestExpense ?? 0;
  const oi = d.otherIncomeNet ?? 0;
  // 3項目すべてnullの場合はincomeBeforeTaxとoperatingIncomeの差分で算出
  if (d.interestIncome == null && d.interestExpense == null && d.otherIncomeNet == null) {
...
// セグメント別売上を抽出（Teslaの固定3セグメント）
function getSegments(seg) {
  if (!seg) return null;
  const result = {};
  if (seg.automotive != null) result.automotive = seg.automotive;
  if (seg.energyGenerationAndStorage != null) result.energyGenerationAndStorage = seg.energyGenerationAndStorage;
  if (seg.servicesAndOther != null) result.servicesAndOther = seg.servicesAndOther;
  return Object.keys(result).length > 0 ? result : null;
}

// セグメント別損益を抽出（Tesla: セグメントごとのrevenue/cost/grossProfit）
function getSegmentProfit(segProfit) {
  if (!segProfit) return null;
  const result = {};
  // segment-profit.json のフィールド:
  //   automotiveRevenue, automotiveCost, automotiveGrossProfit,
  //   energyRevenue, energyCost, energyGrossProfit,
  //   servicesRevenue, servicesCost, servicesGrossProfit, totalGrossProfit
  const segments = [
    { key: 'automotive', prefix: 'automotive' },
    { key: 'energy', prefix: 'energy' },
    { key: 'services', prefix: 'services' },
  ];
  for (const { key, prefix } of segments) {
    if (segProfit[prefix + 'Revenue'] != null || segProfit[prefix + 'GrossProfit'] != null) {
      result[key] = {
        revenue: segProfit[prefix + 'Revenue'] ?? null,
        operatingIncome: segProfit[prefix + 'GrossProfit'] ?? null, // Teslaはセグメント営業利益ではなく粗利益を報告
      };
    }
  }
  return Object.keys(result).length > 0 ? result : null;
...
for (const fyStr of fys) {
  ...
  for (const q of ['Q1', 'Q2', 'Q3', 'Q4']) {
    ...
    quarters.push({
      ...
      // 投資（Tesla: ネットキャッシュポジション）
      investments: inv ? {
        cashAndInvestments: inv.cashAndInvestments ?? null,
        totalDebt: inv.totalDebt ?? null,
