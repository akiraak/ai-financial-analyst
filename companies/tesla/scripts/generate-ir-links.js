// Tesla, Inc. IR決算資料リンクの生成スクリプト
// financials.json から四半期一覧を取得し、各四半期のプレスリリース・10-Q/10-K・IRページへの
// リンクを生成して docs/tesla/ir-links.json に出力する。
// Tesla は個別のADSH番号を持たないため、すべてTesla IRページへのリンクとする。
...
const OUTPUT = path.join(ROOT, 'docs/tesla/ir-links.json');
...
// Tesla IR ページURL
const TESLA_IR_URL = 'https://ir.tesla.com/sec-filings';
...
function generate() {
  ...
  for (const fyKey of sortedFYs) {
    ...
    for (const qNum of [4, 3, 2, 1]) {
      ...
      // 1. プレスリリース（Tesla IRページへリンク）
      documents.push({
        name: 'Press Release',
        url: TESLA_IR_URL,
        description: `Press Release of ${qNames[qNum - 1]} Quarter ${fy}, Tesla Investor Relations`,
      });
      ...
      documents.push({
        name: formType,
        url: TESLA_IR_URL,
        description: `${formType} of ${qNames[qNum - 1]} Quarter ${fy}, Tesla Investor Relations`,
      });

      // 3. Tesla Investor Relations ページ
      documents.push({
        name: 'Investor Relations',
        url: TESLA_IR_URL,
        description: 'Tesla Investor Relations page',
      });
