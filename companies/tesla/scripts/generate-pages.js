// HTMLページ生成スクリプト（Tesla, Inc.用）
// ワークフロー仕様（analyze-and-visualize.md）に基づき、3つのHTMLファイルを生成する
// - docs/tesla/index.html（ランディングページ）
// - docs/tesla/quarters/index.html（四半期選択ページ）
// - docs/tesla/quarters/template.html（四半期詳細テンプレート）
...
const TICKER = 'TSLA';
const SLUG = 'tesla';
const DESC = 'EV・エネルギー。Automotive・Energy Generation & Storage・Services セグメント別業績を可視化。';
  ...
  <div class="container" id="mainContent">
    ...
    <div class="section">
      ...
      <div class="chart-description">
        <p><span class="label">Automotive:</span> 車両販売（Automotive Sales）、規制クレジット（Regulatory Credits）、リース収入（Automotive Leasing）を含むTesla最大のセグメント。EV販売が中核で、規制クレジットは他社への排出権売却による収益。</p>
        <p><span class="label">Energy Generation and Storage:</span> Megapack・Powerwall等のエネルギー貯蔵システムおよびソーラーパネル事業。近年急成長しており、高い粗利率を実現。</p>
        <p><span class="label">Services and Other:</span> 車両メンテナンス・修理、中古車販売、充電ネットワーク（Supercharger）、保険、商品販売等。</p>
        <p><span class="label">セグメント損益:</span> 各セグメントの売上高・売上原価・粗利益がプレスリリースで開示されている。営業利益はセグメント別には報告されていない。</p>
      </div>
    </div>
