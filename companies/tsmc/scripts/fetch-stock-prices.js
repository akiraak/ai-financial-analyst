...
// TSMCの四半期末日（暦年ベース）
// FY2025/Q1 = 2025年3月末, Q2 = 6月末, Q3 = 9月末, Q4 = 12月末
function getQuarterEndDate(fy, q) {
  const year = parseInt(fy.replace('FY', ''));
  switch (q) {
    case 'Q1': return new Date(year, 2, 31);   // 3月末
    case 'Q2': return new Date(year, 5, 30);   // 6月末
    case 'Q3': return new Date(year, 8, 30);   // 9月末
    case 'Q4': return new Date(year, 11, 31);  // 12月末
    default: return null;
  }
}
...
async function getStockPrice(targetDate) {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/TSM?period1=${period1}&period2=${period2}&interval=1d`;
