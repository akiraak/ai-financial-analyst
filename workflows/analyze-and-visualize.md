...
| 7 | 営業利益 × PER | 散布図 | USD建て＆EPS正の企業 | X軸=営業利益、Y軸=PER。ティッカーラベル付き |
| 8 | 営業利益成長率 × PER | 散布図 | EPS正の企業（全通貨） | X軸=営業利益YoY成長率（最大200%）、Y軸=PER。範囲外は注釈表示。成長率は通貨非依存 |

チャート1〜6は値の大きい順にソートして表示。チャート7〜8は散布図のため軸配置で表示。散布図には各点にティッカーラベルを描画（カスタムプラグイン）。
