...
レポートページの先頭（ヘッダー直後、チャート群の前）に、最新の決算ハイライトを解説するサマリーセクションを配置する。

### 記載項目
1. **最新決算の概要** — 最新四半期の売上高・営業利益・純利益・利益率・前年同期比成長率
2. **直近1年の推移** — 直近4四半期のトレンド概況（粗利率の変動、売上拡大ペース等）
3. **セグメント動向** — 主要セグメントの売上・構成比の変化
4. **財務状況** — 現金同等物・FCFの水準
5. **次期ガイダンス** — Outlookデータがある場合、次期の売上見通し

### 形式
- 静的HTMLテキスト（既存のchart-descriptionと同じパターン）
- 主要KPIはハイライト表示（`.kpi-grid` でグリッド配置）
- 新しい四半期データが追加された際に手動で更新する
