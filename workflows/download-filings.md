...
調査結果をユーザーに提示し確認する。ただし**フルビルド一括実行モード中は確認を省略**し、調査結果をログ出力のみで進める。

1. **四半期決算の一覧ページURL** — フルビルド一括実行モード中は確認不要（自動で進む）
2. **取得対象の期間** — `config.json` が存在する場合は `pageYears + chartYears` 年分を自動適用する。`config.json` が無い場合はユーザーに質問する
